...
def train_model():
    ...
    for jdata in json_data:
        ...
        for i in range(0, len(json_data[jdata])):
            x_count = 0
            for x in range(1, 5):
                X_train[y_count][0+x_count] = float(json_data[jdata][str(i)][str(x)]["MPA"])
                X_train[y_count][1+x_count] = float(json_data[jdata][str(i)][str(x)]["PIA"])
                X_train[y_count][2+x_count] = float(json_data[jdata][str(i)][str(x)]["IPA"])
                x_count = x_count + 3
            if jdata == "none":
